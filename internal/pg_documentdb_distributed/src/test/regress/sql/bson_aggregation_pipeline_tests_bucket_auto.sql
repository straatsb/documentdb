...
-- 'n' in $bucketAuto output accumulators must be constant
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$minN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "input": "$year" } } } } } ] }');
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$maxN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "input": "$year" } } } } } ] }');
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$firstN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "input": "$year" } } } } } ] }');
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$lastN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "input": "$year" } } } } } ] }');
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$topN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "output": ["$year", "$_id"], "sortBy": { "$year": 1 } } } } } } ] }');
SELECT document FROM documentdb_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "dollarBucketAuto", "pipeline": [ { "$bucketAuto": { "groupBy": "$price", "buckets": 3, "output": { "smallestPrinces": { "$bottomN": { "n": { "$cond": { "if": { "$eq": [ "$product", "melon" ] }, "then": 2, "else": 1 } }, "output": ["$year", "$_id"], "sortBy": { "$year": 1 } } } } } } ] }');
