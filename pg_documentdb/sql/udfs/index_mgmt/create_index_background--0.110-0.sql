...
CREATE OR REPLACE FUNCTION __API_SCHEMA_INTERNAL__.reindex_index_background(
    p_database_name text, 
    p_reindex_spec __CORE_SCHEMA__.bson, 
    OUT retval __CORE_SCHEMA__.bson,
    OUT ok boolean,
    OUT requests __CORE_SCHEMA__.bson)
RETURNS record
 LANGUAGE C
 VOLATILE
AS 'MODULE_PATHNAME', $$command_reindex_index_background$$;

DROP FUNCTION IF EXISTS __API_SCHEMA_INTERNAL__.reindex_indexes_background_internal CASCADE;
CREATE OR REPLACE FUNCTION __API_SCHEMA_INTERNAL__.reindex_indexes_background_internal(
    IN p_database_name text,
    IN p_arg __CORE_SCHEMA__.bson)
 RETURNS __CORE_SCHEMA__.bson
 LANGUAGE C
 VOLATILE
AS 'MODULE_PATHNAME', $$command_reindex_index_background_internal$$;

CREATE OR REPLACE PROCEDURE documentdb_api_internal_readwrite.build_index_concurrently(IN p_job_index int)
 LANGUAGE C
AS 'MODULE_PATHNAME', $procedure$command_build_index_concurrently$procedure$;
COMMENT ON PROCEDURE documentdb_api_internal_readwrite.build_index_concurrently(int)
    IS 'Builds a index for a collection';

CREATE OR REPLACE FUNCTION documentdb_api_internal_readonly.check_build_index_status(
    IN p_arg __CORE_SCHEMA__.bson,
    OUT retval __CORE_SCHEMA__.bson,
    OUT ok boolean,
    OUT complete boolean)
 ...
 VOLATILE
AS 'MODULE_PATHNAME', $$command_check_build_index_status$$;
COMMENT ON FUNCTION documentdb_api_internal_readonly.check_build_index_status(__CORE_SCHEMA__.bson)
    IS 'Calls check_build_index_status_internal.';

CREATE OR REPLACE FUNCTION documentdb_api_v2.create_indexes_background(
    p_database_name text, 
    p_index_spec __CORE_SCHEMA__.bson, 
    OUT retval __CORE_SCHEMA__.bson,
    OUT ok boolean,
...
RETURNS record
 ...
 VOLATILE
AS 'MODULE_PATHNAME', $$command_create_indexes_background$$;
COMMENT ON FUNCTION documentdb_api_v2.create_indexes_background(text, __CORE_SCHEMA__.bson)
    IS 'Submits the build index(es) requests on a collection and waits for them to finish.';

DROP FUNCTION IF EXISTS documentdb_api_internal_readwrite.create_indexes_background_internal CASCADE;
CREATE OR REPLACE FUNCTION documentdb_api_internal_readwrite.create_indexes_background_internal(
    IN p_database_name text,
    IN p_arg __CORE_SCHEMA__.bson)
 RETURNS __CORE_SCHEMA__.bson
 LANGUAGE C
 VOLATILE
AS 'MODULE_PATHNAME', $$command_create_indexes_background_internal$$;
COMMENT ON FUNCTION documentdb_api_internal_readwrite.create_indexes_background_internal(text,__CORE_SCHEMA__.bson)
    IS 'Queues the Index creation request(s) on a collection';

CREATE OR REPLACE FUNCTION documentdb_api_internal_admin.reindex_index_background(
    p_database_name text, 
    p_reindex_spec __CORE_SCHEMA__.bson, 
    OUT retval __CORE_SCHEMA__.bson,
    OUT ok boolean,
...
DROP FUNCTION IF EXISTS documentdb_api_internal_admin.reindex_indexes_background_internal CASCADE;
CREATE OR REPLACE FUNCTION documentdb_api_internal_admin.reindex_indexes_background_internal(
    IN p_database_name text,
    IN p_arg __CORE_SCHEMA__.bson)
 RETURNS __CORE_SCHEMA__.bson
 LANGUAGE C
...
CREATE OR REPLACE PROCEDURE documentdb_api_v2.create_indexes(IN p_database_name text, 
                                                        IN p_arg __CORE_SCHEMA__.bson,
                                                        INOUT retval __CORE_SCHEMA__.bson DEFAULT null,
                                                        INOUT ok boolean DEFAULT false)
AS $procedure$
BEGIN
  -- We skip other utility hooks as mentioned above, so we shouldn't ever reach here
  RAISE EXCEPTION 'could not create any indexes, possibly due to improper installation' USING ERRCODE = 'internal_error';
END;
$procedure$
LANGUAGE plpgsql;
COMMENT ON PROCEDURE documentdb_api_v2.create_indexes(text, __CORE_SCHEMA__.bson, __CORE_SCHEMA__.bson, boolean)
    IS 'create index(es) on a Mongo collection';