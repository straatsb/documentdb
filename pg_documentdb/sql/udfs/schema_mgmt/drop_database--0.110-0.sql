...
BEGIN
    ...
    IN
      ...
      WHERE
        database_name = p_database_name
    LOOP
        PERFORM documentdb_api_v2.drop_collection(v_collection.database_name, v_collection.collection_name);
    END LOOP;
END;
$fn$ LANGUAGE plpgsql;
COMMENT ON FUNCTION documentdb_api_v2.drop_database(text,__CORE_SCHEMA_V2__.bson)
    IS 'drops a logical document database';