...
/*
 * documentdb_api_v2.insert_one is a convenience function for inserting a single
 * single document.
 */
CREATE OR REPLACE FUNCTION documentdb_api_v2.insert_one(
    p_database_name text,
    p_collection_name text,
    p_document __CORE_SCHEMA_V2__.bson,
    p_transaction_id text default null)
...
BEGIN
    ...
    SELECT p_result INTO p_resultBson FROM documentdb_api_v2.insert(
      p_database_name,
      v_insert,
      p_document::bsonsequence,
      p_transaction_id);
...
END;
$fn$ LANGUAGE plpgsql;
COMMENT ON FUNCTION documentdb_api_v2.insert_one(text,text,__CORE_SCHEMA_V2__.bson,text)
    IS 'insert one document into a collection';
